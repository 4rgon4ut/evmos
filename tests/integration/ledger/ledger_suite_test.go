...
func (suite *LedgerTestSuite) SetupTest() {
	var err error
	if err := s.tryOpenLedger(); err == nil {
		s.useRealDevice = true
	}
...
func (suite *LedgerTestSuite) tryOpenLedger() error {
...
func (suite *LedgerTestSuite) NewKeyringAndCtxs(krHome string, input io.Reader, encCfg params.EncodingConfig) (keyring.Keyring, client.Context, context.Context) {
	var opt keyring.Option
	if !s.useRealDevice {
		opt = s.MockKeyringOption()
	} else {
		opt = evmoskeyring.Option()
	}
	kr, err := keyring.New(
